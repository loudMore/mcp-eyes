...
```bash
pip install "mcp-eyes[resize] @ git+https://github.com/loudMore/vision-extension.git#subdirectory=mcp-vision-extension"
```
...
> *"Install mcp-eyes from `https://github.com/loudMore/vision-extension`. I want to use the `<doubao | openai | qwen | gemini | ollama | …>` provider. Here's my API key: `<KEY>`."*
...
> *"用 https://github.com/loudMore/vision-extension 这个 MCP 给你装上眼睛。视觉模型用 `<豆包 | openai | 通义 | 智谱 | ollama | …>`，API key 是 `<KEY>`。"*
