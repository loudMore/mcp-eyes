...
> **Give text-only reasoning LLMs vision capability.**
...
- **Describe-only protocol** — the vision model is system-prompted into a pure visual scanner. No advice. No fixes. No opinions. Just verbatim transcription and structured description. Reasoning stays with your reasoning model.
...
> *"Install vision-extension from `https://github.com/loudMore/vision-extension`. I want to use the `<doubao | openai | qwen | gemini | ollama | …>` provider. Here's my API key: `<KEY>`."*
...
python -m vision_extension presets

# 2. Generate .mcp.json (writes a server entry)
python -m vision_extension config --preset doubao --api-key sk-... --merge --out .mcp.json

# 3. Generate the CLAUDE.md snippet that tells future agents in this project how to call vision-extension
python -m vision_extension init --lang zh --out CLAUDE.md.vision-section

# 4. Self-check
python -m vision_extension doctor --image C:/path/to/test.png
```
...
|---|---|---|---|
| `VISION_EXTENSION_PROTOCOL` | ✓ | — | `openai` or `anthropic`. See [Choosing a protocol](#choosing-a-protocol). |
| `VISION_EXTENSION_BASE_URL` | ✓ | — | API endpoint, no trailing slash, no `/chat/completions` or `/messages` suffix. |
| `VISION_EXTENSION_API_KEY` | ✓ | — | Provider API key. |
| `VISION_EXTENSION_MODEL` | ✓ | — | Vision model identifier (e.g. `doubao-seed-2.0-pro`, `gpt-4o-mini`). |
| `VISION_EXTENSION_LANG` | | `auto` | `auto` / `en` / `zh`. `auto` detects from the question text. |
| `VISION_EXTENSION_MAX_IMAGE_DIM` | | `2048` | Auto-resize longest edge to this many px (Pillow required). `0` disables. |
| `VISION_EXTENSION_MAX_TOKENS` | | `1536` | Vision model `max_tokens`. |
| `VISION_EXTENSION_TEMPERATURE` | | `0.2` | Sampling temperature. Lower = more deterministic transcription. |
| `VISION_EXTENSION_TIMEOUT` | | `90` | HTTP timeout, seconds. |
| `VISION_EXTENSION_CACHE_ENABLED` | | `true` | Disk cache for (image-hash + prompt + model). |
| `VISION_EXTENSION_CACHE_DIR` | | `~/.cache/vision-extension` | Cache location. |
| `VISION_EXTENSION_ANTHROPIC_VERSION` | | `2023-06-01` | Anthropic protocol only. The `anthropic-version` header value. |

You can also use `python -m vision_extension config` (see above) to generate `.mcp.json` so you never hand-write any of these.

### Choosing a protocol

vision-extension speaks two HTTP protocols. Pick whichever the provider you want to use exposes.
  ...
  "mcpServers": {
    "vision-extension": {
      ...
      "env": {
        ...
        "VISION_EXTENSION_BASE_URL": "https://ark.cn-beijing.volces.com/api/coding/v3",
        "VISION_EXTENSION_API_KEY": "sk-...",
        "VISION_EXTENSION_MODEL": "doubao-seed-2.0-pro"
      }
    }
  }
}
...
After wiring the server, run **once** in each project to generate a CLAUDE.md / AGENTS.md snippet so future agents working in this project know vision-extension is available:

```bash
python -m vision_extension init --lang zh --out CLAUDE.md.vision-section
```

The output reads the configured `VISION_EXTENSION_*` env vars and produces a Markdown block you paste into your project's `CLAUDE.md`. The default snippet is **model-agnostic** — it doesn't matter which text reasoning model you happen to be running, the describe-only contract works the same for all of them.

If you want the snippet to mention your specific model (cosmetic, optional), add `--reasoning-model "<your-name>"` — e.g. `deepseek-v4-pro`, `glm-5.2`, `kimi-k2`, `qwen3-max`. Skip it otherwise.
...
The result reads like a structured scanner log, which is exactly what a reasoning model wants as input. See `src/vision_extension/prompts.py` for the full text.
...
仓库 [`skills/vision-extension/`](skills/) 是一个综合性的 Claude Code skill，覆盖全生命周期：

- **安装阶段**：触发于"装 vision-extension / 接视觉模型 / 粘 GitHub 链接"，按 7 步确定流程让用户只出 API key 就装好
- **日常调用**：每次要调 vision-extension 工具或用户发图片路径时触发，教主模型怎么跟 describe-only 视觉模型高效协作（选哪个 `scene`、`question` 怎么写、何时用 `compare_images`、何时**不该**调、答得不对怎么救）

```bash
cp -r skills/vision-extension ~/.claude/skills/
```
...
This repo ships a single comprehensive [Claude Code skill](skills/) at `skills/vision-extension/` that handles the full lifecycle:

- **Install & configure**: triggered by "install vision-extension" / pasting the GitHub URL → walks the agent through the deterministic 7-step CLI install. User only provides the vision API key.
- **Daily use**: triggered any time the agent is about to call vision-extension tools or the user sends an image path → teaches how to talk to the describe-only vision model effectively (which `scene`, how to phrase `question`, when to batch with `compare_images`, when **not** to call vision, and how to recover from "Not visible in image" responses).
...
`vision-extension` 一次解决两个：

- **一个 MCP 服务**，Claude Code / Cursor / Continue / Cline 通用。
- **describe-only 协议** —— 视觉模型被系统提示锁成纯扫描器。不给建议、不给方案、不给评价，只做逐字转录和结构化描述。推理留给你的推理模型。
...
> *"用 https://github.com/loudMore/vision-extension 这个 MCP 给你装上视觉。视觉模型用 `<豆包 | openai | 通义 | 智谱 | ollama | …>`，API key 是 `<KEY>`。"*
...
python -m vision_extension config --preset doubao --api-key ark-... --merge --out .mcp.json

# 3. 生成 CLAUDE.md 片段（模型无关，不需要填主模型名）
python -m vision_extension init --lang zh --out CLAUDE.md.vision-section

# 4. 自检
python -m vision_extension doctor --image C:/路径/test.png
```
...
| 变量 | 必填 | 默认值 | 说明 |
|---|---|---|---|
| `VISION_EXTENSION_PROTOCOL` | ✓ | — | `openai` 或 `anthropic`，见下文 [选哪个协议](#选哪个协议) |
| `VISION_EXTENSION_BASE_URL` | ✓ | — | API 端点，**不带**结尾斜杠，**不要**带 `/chat/completions` 或 `/messages` 后缀 |
| `VISION_EXTENSION_API_KEY` | ✓ | — | 视觉模型的 API key |
| `VISION_EXTENSION_MODEL` | ✓ | — | 视觉模型 ID（如 `doubao-seed-2.0-pro`、`gpt-4o-mini`）|
| `VISION_EXTENSION_LANG` |  | `auto` | `auto` / `en` / `zh`，`auto` 按 question 文本自动判断 |
| `VISION_EXTENSION_MAX_IMAGE_DIM` |  | `2048` | 自动把长边缩到这么多像素（需 Pillow），`0` 关闭 |
| `VISION_EXTENSION_MAX_TOKENS` |  | `1536` | 视觉模型 `max_tokens` |
| `VISION_EXTENSION_TEMPERATURE` |  | `0.2` | 采样温度，越低转录越稳 |
| `VISION_EXTENSION_TIMEOUT` |  | `90` | HTTP 超时（秒）|
| `VISION_EXTENSION_CACHE_ENABLED` |  | `true` | 本地磁盘缓存（图 hash + prompt + 模型）|
| `VISION_EXTENSION_CACHE_DIR` |  | `~/.cache/vision-extension` | 缓存目录 |
| `VISION_EXTENSION_ANTHROPIC_VERSION` |  | `2023-06-01` | 仅 anthropic 协议用，发到 `anthropic-version` header |

也可以直接用 `python -m vision_extension config`（见上面）让 CLI 自动生成 `.mcp.json`，不用自己填这些。

### 选哪个协议

vision-extension 走两种 HTTP 协议，选你的视觉服务支持的那一个。
...
```json
"VISION_EXTENSION_PROTOCOL": "anthropic",
"VISION_EXTENSION_BASE_URL": "https://ark.cn-beijing.volces.com/api/coding"
```

### 每个项目跑一次的初始化

接好 server 之后，**在每个要用 vision-extension 的项目里跑一次**，自动生成 CLAUDE.md / AGENTS.md 片段告诉项目里跑的 agent "这里有视觉能力可用"：

```bash
python -m vision_extension init --lang zh --out CLAUDE.md.vision-section
```

输出会读 `VISION_EXTENSION_*` 环境变量自动填好视觉模型/协议/base URL，然后生成一段 Markdown 粘进你项目的 `CLAUDE.md`。**默认是模型无关的** —— describe-only 协议对所有文本推理模型一视同仁，不需要告诉它你的主模型是谁。

需要让片段里点名你的主模型（纯装饰，不影响功能）就加 `--reasoning-model "<名字>"`，比如 `deepseek-v4-pro` / `glm-5.2` / `kimi-k2` / `qwen3-max`。不需要就别加。
...
输出会像扫描仪日志：精简、客观、详尽。完整提示词见 `src/vision_extension/prompts.py`。
